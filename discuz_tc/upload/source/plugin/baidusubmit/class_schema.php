...
class BaiduPostSchema
{
    ...
    {
        $this->_postContent = diconv(baidu_strip_invalid_xml(trim($content)), CHARSET, 'utf-8');
    }
...
class BaiduAttachmentSchema
{
    ...
    {
        $this->_attachmentName = diconv(baidu_strip_invalid_xml(trim($name)), CHARSET, 'utf-8');
    }
    public function setUrl($url)
    {
        $this->_attachmentUrl = baidu_encode_url(trim($url));
        $this->_setType(strtolower(fileext($this->_attachmentUrl)));
    }
...
class BaiduThreadSchema
{
    ...
    {
        $this->_forumName = diconv(baidu_strip_invalid_xml(trim($name)), CHARSET, 'utf-8');
    }
    ...
    {
        $this->_threadTitle = diconv(baidu_strip_invalid_xml(trim($title)), CHARSET, 'utf-8');
    }
    ...
    {
        $xml = "<url><loc>{$this->_threadUrl}</loc></url>";
        return $xml;
    }
