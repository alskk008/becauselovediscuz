...
//站长平台回调的URL
$url = $site.'plugin.php?id=baidusubmit:checksign&checksign='.$data['checksign'];
...
if ($output['status'] == 1) {
    ...
    if (empty($old_sppasswd)) {
        ...
        $sign = md5($site.$token);
        //提交全量索引
        $allreturnjson = baidu_submit_sitemap_index('add', 1, $site, $sppasswd, $sign);
        ...
        //提交增量索引
        $incresultjson =  baidu_submit_sitemap_index('add', 2, $site, $sppasswd, $sign);
        ...
        if ($result === 0) {
            ...
            //删除旧索引文件
            if ($old_sppasswd) {
                baidu_submit_sitemap_index('del', 1, $site, $old_sppasswd, $sign);
                baidu_submit_sitemap_index('del', 2, $site, $old_sppasswd, $sign);
            }
            baidu_show_json(array(
                'error' => 0,
